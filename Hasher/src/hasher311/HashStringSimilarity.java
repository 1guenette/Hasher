...
public class HashStringSimilarity
{
	...
	public HashStringSimilarity(String s1p, String s2p, int sLength)
	{
		length = sLength;
		s1 = s1p;
		s2 = s2p;	
	...
	public int pow(int val, int exp) 
	{
	 ...
	 public float lengthOfS1()
		{
		 ArrayList<Tuple> checkedTuple = new ArrayList<Tuple>();
		 double sum = 0;
			for(int i = 0; i<table1.size(); i++) //each table location
			{
				...
				{
					int occur = 0;				
					if(!checkedTuple.contains(current)) 
					{
						occur = table1.search(current);
					}
					sum += occur*occur;
